...
router.post('/', requireAdmin, [
  ...
  try {
    ...
    const { title, description, status, priority, project_name, assignee_ids, label_ids } = req.body;
    const userId = req.userId;
    ...
    const result = await pool.query(
      `INSERT INTO erp.issues (title, description, status, priority, project_name, created_by)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [title, description || null, status || 'open', priority || 'medium', project_name || null, userId]
    );
    ...
    await pool.query(
      `INSERT INTO erp.issue_activity (issue_id, user_id, action, details)
       VALUES ($1, $2, $3, $4)`,
      [issue.id, userId, 'created', JSON.stringify({ title })]
    );
...
router.put('/:id', requireAdmin, async (req, res) => {
  try {
    ...
    const { title, description, status, priority, project_name } = req.body;
    const userId = req.userId;
    ...
    const result = await pool.query(
      `UPDATE erp.issues 
       ...
       WHERE id = $6
       RETURNING *`,
      [title, description, status, priority, project_name, issueId]
    );
